// RedisNodeRegistry.cpp - Redis 节点注册中心实现
...
// 将节点信息序列化为 JSON 字符串
std::string RedisNodeRegistry::serializeNode(const NodeInfo& node)
{
    Json::Value root;
...
bool RedisNodeRegistry::parseNode(const std::string& json, NodeInfo& out)
{
    Json::Reader reader;
    Json::Value root;
    ...
    return !out.client_host.empty() && !out.client_port.empty() &&
           !out.rpc_host.empty() && !out.rpc_port.empty();
}

// 判断节点是否存活（未过期）
bool RedisNodeRegistry::isAlive(const NodeInfo& node)
{
    return node.expire_at >= utils::nowSec();
}

// 清理 Redis 中所有已过期的节点记录及其登录计数
void RedisNodeRegistry::cleanupExpiredNodes()
{
    std::map<std::string, std::string> all;
    if (!RedisMgr::getInstance().hGetAll(RedisPrefix::CHAT_NODES, all))
    {
        Log::warn(LogModule::Registry,
                  "cleanupExpiredNodes: failed to get all nodes from Redis");
        return;
    }
    ...
    for (const auto& entry : all)
    {
        ...
        if (!parseNode(entry.second, node) || !isAlive(node))
        {
            ...
            Log::info(LogModule::Registry,
                      "cleanupExpiredNodes: removed expired node {}", entry.first);
        }
    }
    if (cleaned > 0)
    {
        Log::info(LogModule::Registry,
                  "cleanupExpiredNodes: removed {} expired node(s)", cleaned);
    }
}

// 注册节点：先清理过期节点，若同名节点已存活且 instance_id 不同则拒绝
bool RedisNodeRegistry::registerNode(const NodeInfo& node)
{
    if (node.name.empty())
    {
    ...
    {
        Log::error(LogModule::Registry,
                   "registerNode: failed to save node {} to Redis", node.name);
        return false;
    }

    redis.hSet(RedisPrefix::LOGIN_COUNT, node.name, "0");
    Log::info(LogModule::Registry,
              "registerNode: registered chat node {} instance {}",
              node.name, node.instance_id);
    return true;
}

// 注销节点：清理节点绑定的所有用户路由，再删除节点记录
bool RedisNodeRegistry::unregisterNode(const std::string& name,
                                        const std::string& instance_id)
{
    ...
    {
        Log::info(LogModule::Registry,
                  "unregisterNode: node {} not found, nothing to do", name);
        return true;
    }
...
bool RedisNodeRegistry::heartbeat(const std::string& name,
                                   const std::string& instance_id)
{
    auto existing = getNode(name);
    ...
    bool ok = RedisMgr::getInstance().hSet(
        RedisPrefix::CHAT_NODES, name, serializeNode(updated));
    if (ok)
    {
        Log::debug(LogModule::Registry,
                   "heartbeat: renewed node {} instance {}", name, instance_id);
    }
    else
    {
        Log::error(LogModule::Registry,
                   "heartbeat: failed to update node {} in Redis", name);
    }
    return ok;
}

// 按名称查询节点，若不存在或已过期返回 nullopt
std::optional<NodeInfo> RedisNodeRegistry::getNode(const std::string& name)
{
    ...
    {
        Log::debug(LogModule::Registry,
                   "getNode: node {} not found in Redis", name);
        return std::nullopt;
    }
    ...
    {
        Log::debug(LogModule::Registry,
                   "getNode: node {} is expired or invalid", name);
        return std::nullopt;
    }
    return node;
}
...
// 列出 Redis 中所有存活的节点
std::vector<NodeInfo> RedisNodeRegistry::listNodes()
{
    ...
    {
        Log::warn(LogModule::Registry,
                  "listNodes: failed to get all nodes from Redis");
        return result;
    }
    ...
    Log::info(LogModule::Registry,
              "listNodes: found {} alive node(s)", result.size());
    return result;
}

// 查询用户当前绑定的节点
std::optional<NodeInfo> RedisNodeRegistry::getNodeForUser(int uid)
{
    std::string nodeName;
    if (!RedisMgr::getInstance().get(
            std::string(RedisPrefix::USER_CHAT_NODE) + std::to_string(uid),
            nodeName) ||
        nodeName.empty())
    {
        Log::debug(LogModule::Registry,
                   "getNodeForUser: user {} not bound to any node", uid);
        return std::nullopt;
    }
    ...
    {
        Log::debug(LogModule::Registry,
                   "getNodeForUser: user {} -> node {}", uid, nodeName);
    }
    else
    {
        Log::warn(LogModule::Registry,
                  "getNodeForUser: user {} bound to node {} but node is unavailable",
                  uid, nodeName);
    }
    return node;
}
...
bool RedisNodeRegistry::bindUser(int uid, const std::string& node_name)
{
    ...
    {
        Log::warn(LogModule::Registry,
                  "bindUser: target node {} is not alive for user {}",
                  node_name, uid);
        return false;
    }
    ...
    std::string oldNode;
    if (redis.get(std::string(RedisPrefix::USER_CHAT_NODE) + uidStr, oldNode) &&
        !oldNode.empty() && oldNode != node_name)
    {
        redis.sRem(std::string(RedisPrefix::CHAT_NODE_USERS) + oldNode, uidStr);
        Log::info(LogModule::Registry,
                  "bindUser: moved user {} from old node {} to {}",
                  uid, oldNode, node_name);
    }
    ...
    Log::info(LogModule::Registry,
              "bindUser: user {} bound to node {}", uid, node_name);
    return true;
}

// 解绑用户与节点的绑定关系
bool RedisNodeRegistry::unbindUser(int uid)
{
    if (uid <= 0)
    {
    ...
    std::string nodeName;
    if (redis.get(std::string(RedisPrefix::USER_CHAT_NODE) + uidStr, nodeName) &&
        !nodeName.empty())
    {
        redis.sRem(std::string(RedisPrefix::CHAT_NODE_USERS) + nodeName, uidStr);
        Log::info(LogModule::Registry,
                  "unbindUser: removed user {} from node {}", uid, nodeName);
    }
...
std::optional<NodeInfo> RedisNodeRegistry::selectLeastLoadedNode()
{
    auto nodes = listNodes();
    if (nodes.empty())
    {
        Log::warn(LogModule::Registry,
                  "selectLeastLoadedNode: no alive nodes available");
        return std::nullopt;
    }
    ...
    {
        auto countStr =
            RedisMgr::getInstance().hGet(RedisPrefix::LOGIN_COUNT, node.name);
